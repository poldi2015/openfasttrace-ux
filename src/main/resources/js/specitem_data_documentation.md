# Specitems Array Documentation

## Overview

The `specitems` array is generated by the JSGenerator class in OpenFastTrace and contains structured data representing
specification items and their traceability information. This data is used by OpenFastTrace-UX for interactive
visualization and analysis of specification coverage.
...
The `covered` array contains Coverage enum IDs per type, where each position corresponds to a specItem type in the
`project.types` array. The Coverage enum values are:
...
- **3 = MISSING**: These coverage type are needed by this specItems or specItems that cover this specItem (deep
  coverage) but are not covered

For example, if `project.types = ["itest", "feat", "fea", "req", "arch", "utest"]` and `covered = [0, 0, 2, 2, 1, 3]`,
this means:
